    ...
    try:
        ...
        if dialect != 'postgresql':
            with op.batch_alter_table('hashmap_thresholds') as batch_op:
                if dialect != 'sqlite':
                    batch_op.drop_constraint(u'uniq_field_mapping',
                                            type_='unique')
                    batch_op.drop_constraint(u'uniq_service_mapping',
                                            type_='unique')
                batch_op.create_unique_constraint('uniq_field_threshold',
                                                  ['level', 'field_id'])
                batch_op.create_unique_constraint('uniq_service_threshold',
                                                  ['level', 'service_id'])
        ...
        if dialect == 'sqlite':
            ...
            op.drop_constraint(u'uniq_field_mapping',
                            'hashmap_thresholds',
                            type_='unique')
            op.drop_constraint(u'uniq_service_mapping',
                            'hashmap_thresholds',
                            type_='unique')
            op.create_unique_constraint('uniq_field_threshold',
                                        'hashmap_thresholds',
                                        ['level', 'field_id'])
            op.create_unique_constraint('uniq_service_threshold',
                                        'hashmap_thresholds',
                                        ['level', 'service_id'])
...
def downgrade():
    ...
    try:
        if dialect != 'postgresql':
            with op.batch_alter_table('hashmap_thresholds') as batch_op:
                if dialect != 'sqlite':
                    batch_op.drop_constraint('uniq_service_threshold',
                                            type_='unique')
                    batch_op.drop_constraint('uniq_field_threshold',
                                            type_='unique')
                batch_op.create_unique_constraint('uniq_field_mapping',
                                                  ['level', 'field_id'])
                batch_op.create_unique_constraint('uniq_service_mapping',
                                                  ['level', 'service_id'])
    except AttributeError:
        # No support for batch operations
        if dialect == 'sqlite':
            old_table = create_table(True)
            sel = sa.sql.expression.select(old_table.columns.keys())
            op.execute(
                old_table.insert().from_select(old_table.columns.keys(), sel.select_from('hashmap_thresholds')))
            op.drop_table('hashmap_thresholds')
            op.rename_table('tmig_hashmap_thresholds', 'hashmap_thresholds')
        elif dialect != 'postgresql':
            op.drop_constraint('uniq_service_threshold',
                            'hashmap_thresholds',
                            type_='unique')
            op.drop_constraint('uniq_field_threshold',
                            'hashmap_thresholds',
                            type_='unique')
            op.create_unique_constraint(u'uniq_service_mapping',
                                        'hashmap_thresholds',
                                        ['level', 'service_id'])
            op.create_unique_constraint(u'uniq_field_mapping',
                                        'hashmap_thresholds',
                                        ['level', 'field_id'])
